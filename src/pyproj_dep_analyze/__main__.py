...
Purpose
-------
Provide the ``python -m pyproj_dep_analyze`` path mandated by the
project's packaging guidelines. The wrapper delegates to
:func:`pyproj_dep_analyze.cli.main` so that module execution mirrors the
...
def _module_main() -> int:
    ...
    Implements ``python -m pyproj_dep_analyze`` by delegating to the
