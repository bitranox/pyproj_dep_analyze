...
By delegating to :mod:`pyproj_dep_analyse.behaviors` the transport stays
...
targets register the console script defined in :mod:`pyproj_dep_analyse.__init__conf__`.
...
from .analyzer import run_analysis, write_outdated_json
...
def cli_config(format: str, section: str | None) -> None:
    """Display the current merged configuration from all sources.

    Shows configuration loaded from:
    - Default config (built-in)
    - Application config (/etc/xdg/pyproj-dep-analyse/config.toml)
    - User config (~/.config/pyproj-dep-analyse/config.toml)
    - .env files
    - Environment variables (PYPROJ_DEP_ANALYSE_*)
...
@click.option(
    ...
    envvar="GITHUB_TOKEN",
    default=None,
    help="GitHub token for API authentication. Can also be set via PYPROJ_DEP_ANALYSE_GITHUB_TOKEN env var or config file.",
)
@click.option(
    "--timeout",
    type=float,
    default=None,
    help="Request timeout in seconds. Default from config (30.0). Can also be set via PYPROJ_DEP_ANALYSE_TIMEOUT env var.",
)
@click.option(
    "--concurrency",
    type=int,
    default=None,
    help="Maximum concurrent API requests. Default from config (10). Can also be set via PYPROJ_DEP_ANALYSE_CONCURRENCY env var.",
)
@click.option(
    "--format",
    "output_format",
...
def cli_analyze(
    ...
    - Environment variables (PYPROJ_DEP_ANALYSE_*)
    - Config files (~/.config/pyproj-dep-analyse/config.toml)
    ...
    Examples:
        ...
        $ pyproj-dep-analyse analyze path/to/pyproject.toml -o results.json

        # Show summary only
        $ pyproj-dep-analyse analyze --format summary

        # Use environment variables for settings
        $ PYPROJ_DEP_ANALYSE_GITHUB_TOKEN=ghp_xxx pyproj-dep-analyse analyze
    """
    ...
    with lib_log_rich.runtime.bind(job_id="cli-analyze", extra={"command": "analyze"}):
...
def cli_config_deploy(targets: tuple[str, ...], force: bool) -> None:
    ...
    Examples:
        ...
        $ pyproj-dep-analyse config-deploy --target app --target user
