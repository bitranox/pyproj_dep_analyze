...
from .models import DependencyInfo, PythonVersion
from .schemas import GitHubReleaseSchema, GitHubTagSchema, PyPIResponseSchema
...
class VersionResolver:
    ...
    def _get_headers(self, for_github: bool = False) -> dict[str, str]:
        ...
        headers = {"Accept": "application/json", "User-Agent": "pyproj-dep-analyse/1.0"}
    ...
    async def resolve_pypi_async(
        ...
        """Resolve the latest version of a package from PyPI."""
        ...
        try:
            result = await self._fetch_pypi_version(package_name)
    ...
    async def _fetch_pypi_version(self, package_name: str) -> VersionResult:
        """Fetch version from PyPI API."""
        url = PYPI_API_URL.format(package=package_name)
        try:
            return await self._query_pypi_api(url, package_name)
    ...
    async def _query_pypi_api(self, url: str, package_name: str) -> VersionResult:
        ...
        return self._parse_pypi_response(response)

    def _parse_pypi_response(self, response: httpx.Response) -> VersionResult:
        """Parse PyPI API response into VersionResult."""
        pypi_response = PyPIResponseSchema.model_validate(response.json())
        latest = pypi_response.info.version
        if latest:
            return VersionResult(latest_version=latest)
        return VersionResult(is_unknown=True, error="No version info in PyPI response")
