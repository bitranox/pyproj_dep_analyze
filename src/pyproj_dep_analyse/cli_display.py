...
from .analyzer import entry_to_dict
from .models import Action, AnalysisResult, OutdatedEntry
...
def display_json(result: AnalysisResult) -> None:
    ...
    """
    data = [entry_to_dict(entry) for entry in result.entries]
    click.echo(json.dumps(data, indent=2))


def display_analysis_results(result: AnalysisResult, output_format: str) -> None:
    """Display analysis results in the specified format.

    Args:
        result: The analysis result to display.
        output_format: One of "summary", "table", or "json".
    """
