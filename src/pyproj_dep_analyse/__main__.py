...
Purpose
-------
Provide the ``python -m pyproj_dep_analyse`` path mandated by the
project's packaging guidelines. The wrapper delegates to
:func:`pyproj_dep_analyse.cli.main` so that module execution mirrors the
...
def _module_main() -> int:
    ...
    Implements ``python -m pyproj_dep_analyse`` by delegating to the
